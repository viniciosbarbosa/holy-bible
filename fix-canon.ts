...
const filePath = path.resolve('./src/features/bible-custom/constants/canon-data.ts');
const content = fs.readFileSync(filePath, 'utf8');

// We need to extract CANON_DATA and CANON_DATA_ENGLISH from the file.
// Since we are running in TS, we can just import them!
import { CANON_DATA, CANON_DATA_ENGLISH } from './src/features/bible-custom/constants/canon-data.js';
...
const getEnBook = (phaseId: string, bookId: string) => {
  const phase = enPhaseMap.get(phaseId);
  if (!phase) return null;
  return phase.books.find((b: any) => b.id === bookId);
};
...
console.log('Fixed CANON_DATA_ENGLISH successfully!');
